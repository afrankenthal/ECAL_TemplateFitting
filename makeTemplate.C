...
Double_t fitf2(Double_t * x, Double_t * par) {
    ...
    double arg2 = (x[0] - par[3]);///par[5];
    return par[0] * templateSpline->Eval(arg1) + par[2] * templateSpline->Eval(arg2) + par[4];
}
...
void makeTemplate() {
    ...
    for (int j = 0; j < 1024; j++) templateVec[j] = 0;
    int numTraces = 0;
    ...
    for (int j = 0; j < 1024; j++) templateVec[j] /= numTraces;
    ...
    for (int j = 0; j < 1024; j++) {
        hTemplate->SetBinContent(j, templateVec[j]);
        // std::cout << "j: " << j << ", template: " << templateVec[j] << std::endl;
    }

    hTemplate->Draw();
    templateSpline = new TSpline3("templateSpline", -1, 1023, templateVec, 1024);
    templateSpline->Draw("LCPSAME");

    std::cout << "Done building template." << std::endl;
    std::cout << "Testing template on training set..." << std::endl;

    TH1F * hAmpl = new TH1F("hAmpls", "hAmpls", 1000, 0, 10);
    TH1F * hStretch = new TH1F("hStretch", "hStretch", 1000, 0, 10);
    TH1F * hOffset = new TH1F("hOffset", "hOffset", 1000, -200, 20);
    ...
    for (int i = 0; i < numEntries; i++) {
        ...
        if (isSignal&&isBTF) {
            ...
            TGraph * testGraph = new TGraph(1024, x, y);
